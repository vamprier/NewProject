...
void CGridCtrl::GetTextList()
{
	int row = GetRowCount();
	int col = GetColumnCount();
	m_textlist=new CStringArray[row];
	int i,j;
	for (i=0;i<row;i++)
	{
		for (j=0;j<col;j++)
		{
			CString str;
			str.Format(_T("%s"),GetItemText(i,j));
			m_textlist[i].Add(str);
		}
	}
}
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	CString strOrg = GetItemText(cellid.row,1);
	CString strCode = GetItemText(cellid.row,2);
	CString strProName = GetItemText(cellid.row,3);
